 ...
 */
  ...
  public async approveSession(proposal: Web3WalletTypes.SessionProposal, currentChainId: string, kernelAddress: string) {
    if (!this.web3Wallet) throw new Error('WalletConnect not initialized')

    const namespaces = this.getNamespaces(proposal, currentChainId, kernelAddress)
  ...
  private getNamespaces(
    ...
    const eip155Accounts = eip155ChainIds.map((eip155ChainId) => `${eip155ChainId}:${kernelAddress}`)

    // Don't include optionalNamespaces methods/events
    const methods = uniq((proposal.params.requiredNamespaces[EIP155]?.methods || []).concat(KERNEL_COMPATIBLE_METHODS))
    const events = uniq((proposal.params.requiredNamespaces[EIP155]?.events || []).concat(KERNEL_COMPATIBLE_EVENTS))
